...
PROCEDURE THojaCalc.LoadProg;
  ...
  BEGIN
    IF ProgLoaded THEN
      CloseProg;
    m_eTipo := thcNone;
    IF (UpperCase(ExtractFileExt(m_strFileName)) = '.XLS') or
       (UpperCase(ExtractFileExt(m_strFileName)) = '.XLSX') THEN
  ...
  BEGIN
    IF DocLoaded THEN
      BEGIN
        ...
        IF IsOpenOffice THEN
          result := m_vActiveSheet.getCellByPosition(col - 1, row - 1).getString;
      END {IF};
    ...
    IF DocLoaded THEN
      BEGIN
        IF IsExcel THEN
          ...
          END {IF};
        IF IsOpenOffice THEN
           m_vActiveSheet.getCellByPosition(col - 1, row - 1):= strTxt;
